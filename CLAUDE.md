...
Delineation-Informed Geographic Ground Selection — golem-based Shiny app
for selecting historic orthophoto imagery from the BC Data Catalogue.
Draw polygons or upload a custom AOI, filter by year/media/scale, view
estimated photo footprints, run resolution-prioritized selection, and
export to CSV.

## Repository Context

**Repository:** NewGraphEnvironment/diggs **Primary Language:** R (golem
Shiny package) **Entry Point:** `app.R` (golem launcher via
[`pkgload::load_all()`](https://pkgload.r-lib.org/reference/load_all.html) +
[`run_app()`](https://newgraphenvironment.github.io/diggs/reference/run_app.md))

## Architecture

### golem package structure

- `DESCRIPTION` — package metadata, Imports (fly, sf, leaflet, DT,
  bslib, golem)
- `NAMESPACE` — generated by roxygen2
- `app.R` — golem launcher (pkgload + run_app)
- `inst/golem-config.yml` — config (data_dir, app_prod)
- `inst/app/www/` — static assets (logo)

### R/

- `R/run_app.R` — exported
  [`run_app()`](https://newgraphenvironment.github.io/diggs/reference/run_app.md),
  accepts `data_dir` override
- `R/app_ui.R` — top-level UI (bslib page_sidebar)
- `R/app_server.R` — top-level server (loads layers, wires modules)
- `R/app_config.R` — `app_sys()` and `get_golem_config()` helpers
- `R/diggs-package.R` — package-level doc and imports
- `R/mod_map.R` — leaflet map module (draw tools, AOI display,
  footprints)
- `R/mod_filters.R` — sidebar filter controls (year, media, scale,
  priority selection)
- `R/mod_table.R` — DT table module with CSV/Excel export
- `R/utils_data.R` — `load_cached_layers()` reads geojson from data_dir
- `R/utils_geo.R` — `validate_geometry()`, `drawn_feature_to_sf()`
...
- Cached geojson layers rebuilt by `data-raw/cache_data.R`

## Ecosystem

- [fly](https://github.com/NewGraphEnvironment/fly) — spatial photo
  operations (footprints, filtering, coverage selection). Used by diggs
  internally via
  [`fly::fly_footprint()`](https://newgraphenvironment.github.io/fly/reference/fly_footprint.html),
  [`fly::fly_filter()`](https://newgraphenvironment.github.io/fly/reference/fly_filter.html),
  [`fly::fly_select()`](https://newgraphenvironment.github.io/fly/reference/fly_select.html)
- [flooded](https://github.com/NewGraphEnvironment/flooded) — floodplain
  delineation (generates AOIs that diggs consumes)
- [drift](https://github.com/NewGraphEnvironment/drift) — satellite land
  cover change within floodplains

## Lessons Learned

- **bcdata** needs record UUIDs or catalogue names, not `SCHEMA.TABLE`
  object names — use `bcdc_search()` to find them
- **Leaflet marker clustering** is essential for 9k+ points — without it
  the map is unusable
- **Viewport-reactive observers cause instability** — don’t react to
  `input$map_bounds` for computing footprints or filtering table data.
  Use button-click triggers instead
- **Cross-module button communication** — pass the reactive through the
  module return list rather than passing the parent `input` object
- **DT `server = FALSE`** required for CSV/Excel export of all rows, not
  just the visible page
- **DT `dom` string controls spacing** — `"Blfrtip"` adds gaps,
  `"Brtip"` is compact
- **Port conflicts** are common when restarting Shiny —
  `lsof -ti:PORT | xargs kill -9`
- **golem config** — `get_golem_config("data_dir")` reads from
  `inst/golem-config.yml`; override at runtime via
  `run_app(data_dir = "...")`

## Workflow Preferences

- Test before commit (`devtools::test()`)
- Run [`shiny::runApp()`](https://rdrr.io/pkg/shiny/man/runApp.html) or
  ...
  to verify UI changes before pushing

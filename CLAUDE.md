...
BC Historic Airphoto Explorer — interactive Shiny app for selecting historic orthophoto imagery from the BC Data Catalogue. Draw polygons or upload a custom AOI, filter by year/media/scale, view estimated photo footprints, and export selections to CSV.

## Repository Context

**Repository:** NewGraphEnvironment/airbc
**Primary Language:** R (Shiny)
**Entry Point:** `app.R`

## Architecture

- `app.R` — top-level UI/server; sources modules and loads cached data at startup
- `R/mod_map.R` — leaflet map module (draw tools, AOI display, photo footprints)
...
- **bcdata** needs record UUIDs or catalogue names, not `SCHEMA.TABLE` object names — use `bcdc_search()` to find them
- **Leaflet marker clustering** is essential for 9k+ points — without it the map is unusable
- **Viewport-reactive observers cause instability** — don't react to `input$map_bounds` for computing footprints or filtering table data. Use button-click triggers instead
- **Cross-module button communication** — pass the reactive through the module return list rather than passing the parent `input` object
- **DT `server = FALSE`** required for CSV/Excel export of all rows, not just the visible page
- **DT `dom` string controls spacing** — `"Blfrtip"` adds gaps, `"Brtip"` is compact
- **Port conflicts** are common when restarting Shiny — `lsof -ti:PORT | xargs kill -9`

## Workflow Preferences

- Test before commit
- Run `shiny::runApp()` to verify UI changes before pushing

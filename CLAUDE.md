...
Delineation-Informed Geographic Ground Selection — golem-based Shiny app for selecting historic orthophoto imagery from the BC Data Catalogue. Draw polygons or upload a custom AOI, filter by year/media/scale, view estimated photo footprints, run resolution-prioritized selection, and export to CSV.

## Repository Context

**Repository:** NewGraphEnvironment/diggs
...
- `DESCRIPTION` — package metadata, Imports (fly, sf, leaflet, DT, bslib, golem)
...
### R/
- `R/run_app.R` — exported `run_app()`, accepts `data_dir` override
...
- `R/mod_map.R` — leaflet map module (draw tools, AOI display, footprints)
- `R/mod_filters.R` — sidebar filter controls (year, media, scale, priority selection)
...
- [fly](https://github.com/NewGraphEnvironment/fly) — spatial photo operations (footprints, filtering, coverage selection). Used by diggs internally via `fly::fly_footprint()`, `fly::fly_filter()`, `fly::fly_select()`
- [flooded](https://github.com/NewGraphEnvironment/flooded) — floodplain delineation (generates AOIs that diggs consumes)
- [drift](https://github.com/NewGraphEnvironment/drift) — satellite land cover change within floodplains
...
- **golem config** — `get_golem_config("data_dir")` reads from `inst/golem-config.yml`; override at runtime via `run_app(data_dir = "...")`

## Workflow Preferences

- Test before commit (`devtools::test()`)
- Run `shiny::runApp()` or `golem::run_dev()` to verify UI changes before pushing
